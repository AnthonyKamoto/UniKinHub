...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Configuration des utilisateurs pour les notifications {frequency}..."
            )
        )

        # Mettre à jour tous les utilisateurs actifs
        users_updated = User.objects.filter(is_active=True).update(
            notification_frequency=frequency, email_notifications=True
        )

        self.stdout.write(
            self.style.SUCCESS(
                f'{users_updated} utilisateurs configurés avec la fréquence "{frequency}"'
            )
        )
    ...
    def create_test_news(self):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"{created_count} nouvelles actualités créées pour les tests"
                )
            )
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"Erreur lors de la création des actualités de test: {str(e)}"
                )
            )
