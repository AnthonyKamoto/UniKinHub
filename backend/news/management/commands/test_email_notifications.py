    ...
    def handle(self, *args, **options):
        ...
        try:
            if notification_type in ["approval", "rejection", "submission"]:
                ...
                if not news:
                    self.stdout.write(
                        self.style.WARNING("Aucun article trouvé pour cet utilisateur")
                    )
                    return
                ...
                if notification_type == "submission":
                    ...
                    success = NotificationService.send_moderation_notification(
                        ...
                        "rejected",
                        moderator,
                        "L'article nécessite des corrections: vérifiez l'orthographe et ajoutez des sources.",
                    )

                if success:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Notification {notification_type} envoyée avec succès à {user.email}"
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.ERROR(
                            f"Échec de l'envoi de la notification {notification_type}"
                        )
                    )
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Digest quotidien envoyé à {count} utilisateurs"
                    )
                )
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Digest hebdomadaire envoyé à {count} utilisateurs"
                    )
                )
