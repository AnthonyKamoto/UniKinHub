...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            self.stdout.write(
                self.style.WARNING(f'Utilisateur modérateur existe déjà: {moderator.username}')
            )
        ...
        categories_data = [
            {'name': 'Académique', 'description': 'Informations académiques', 'color': '#FF9800'},
            {'name': 'Infrastructure', 'description': 'Infrastructures universitaires', 'color': '#2196F3'},
            {'name': 'Événements', 'description': 'Événements du campus', 'color': '#4CAF50'},
            {'name': 'Urgent', 'description': 'Informations urgentes', 'color': '#F44336'},
        ]
        ...
        for news_info in news_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Actualité créée: {news.title}')
                )

        self.stdout.write(
            self.style.SUCCESS('✅ Données de test pour la modération créées avec succès!')
        )
        self.stdout.write(
