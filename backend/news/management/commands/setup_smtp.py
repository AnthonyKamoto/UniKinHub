...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if test_email:
            ...
            self.stdout.write(
                self.style.WARNING(
                    "💡 Utilisez --test-email pour tester la configuration"
                )
            )
    ...
    def show_current_config(self):
        ...
        if settings.EMAIL_BACKEND == "django.core.mail.backends.smtp.EmailBackend":
            self.stdout.write(
                f'   Hôte: {getattr(settings, "EMAIL_HOST", "Non défini")}'
            )
            self.stdout.write(
                f'   Port: {getattr(settings, "EMAIL_PORT", "Non défini")}'
            )
            self.stdout.write(f'   TLS: {getattr(settings, "EMAIL_USE_TLS", False)}')
            self.stdout.write(f'   SSL: {getattr(settings, "EMAIL_USE_SSL", False)}')
            self.stdout.write(
                f'   Utilisateur: {getattr(settings, "EMAIL_HOST_USER", "Non défini")}'
            )
            ...
            self.stdout.write(
                "   📧 Mode console - les emails s'affichent dans la console"
            )
    ...
    def test_smtp_config(self, test_email):
        ...
        if backend == "django.core.mail.backends.console.EmailBackend":
            self.stdout.write(
                self.style.WARNING(
                    "⚠️  Configuration en mode console - les emails s'affichent dans la console"
                )
            )
            return
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "📁 Configuration en mode fichier - les emails seront sauvegardés"
                )
            )
        elif backend != "django.core.mail.backends.smtp.EmailBackend":
            self.stdout.write(self.style.WARNING(f"⚠️  Backend non reconnu: {backend}"))
        ...
        if backend == "django.core.mail.backends.smtp.EmailBackend":
            ...
            if missing_settings:
                self.stdout.write(
                    self.style.ERROR(
                        f'❌ Configuration incomplète. Manquant: {", ".join(missing_settings)}'
                    )
                )
...
Actualités Étudiantes Kinshasa - Système de Notifications
            ...
            if success:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✅ Email de test envoyé avec succès à {test_email}!"
                    )
                )
                ...
                self.stdout.write("\n🚀 Prochaines étapes:")
                self.stdout.write(
                    "   1. Testez les notifications avec les commandes de test"
                )
                self.stdout.write(
                    "   2. Configurez la planification automatique des digest"
                )
                self.stdout.write(
                    "   3. Informez vos utilisateurs du système de notifications"
                )

            else:
                self.stdout.write(
                    self.style.ERROR(
                        "❌ Échec de l'envoi - vérifiez votre configuration"
                    )
                )

        except Exception as e:
            self.stdout.write(self.style.ERROR(f"❌ Erreur SMTP: {str(e)}"))
    ...
    def show_troubleshooting_tips(self):
        ...
        self.stdout.write(
            "   2. Pour Gmail, utilisez un mot de passe d'application (16 caractères)"
        )
        self.stdout.write(
            "   3. Vérifiez que votre fournisseur autorise les connexions SMTP"
        )
