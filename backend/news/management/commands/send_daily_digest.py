...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS('Début de l\'envoi du résumé quotidien...')
        )
        ...
        if options['dry_run']:
            ...
            self.stdout.write(
                self.style.SUCCESS('Simulation terminée')
            )
            return
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'Erreur lors de l\'envoi du résumé quotidien: {str(e)}')
            )