...
from .models import (Category, Departement, Faculte, News, NewsLike, NewsView,
                     Notification, NotificationPreference, Role, Universite,
                     User)
...
class NewsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            "moderator_approved",
            "moderation_comment",
            "desired_publish_start",
            "desired_publish_end",
            "publish_date",
            "expiry_date",
            "admin_invalidated_by",
            "admin_invalidation_reason",
            "invalidated_at",
            "target_universities",
            "target_programs",
            "views_count",
    ...
    def get_time_since(self, obj):
        # Simplified: always return default to avoid errors during creation
        try:
            from django.utils import timezone
            from django.utils.timesince import timesince

            # Try to get a valid date
            date_to_use = getattr(obj, "created_at", None) or timezone.now()
            return timesince(date_to_use, timezone.now())
        except:
            return "à l'instant"

    def get_title(self, obj):
        # Si la news est publiée ou approuvée, retourner le titre final sinon le draft
        if obj.status == "published" or getattr(obj, "moderator_approved", False):
            return obj.final_title or obj.draft_title
        return obj.draft_title

    def get_content(self, obj):
        if obj.status == "published" or getattr(obj, "moderator_approved", False):
            return obj.final_content or obj.draft_content
        return obj.draft_content


class NewsCreateSerializer(serializers.ModelSerializer):
    """Sérialiseur pour la création d'actualités par les publiants"""

    class Meta:
        model = News
        # Création utilise les champs draft et les dates souhaitées de publication
        fields = (
            "programme_ou_formation",
            "draft_title",
            "draft_content",
    ...
    def create(self, validated_data):
        ...
        # Déterminer le statut initial selon les permissions
        # Vérifier d'abord l'ancien système de rôles (plus fiable)
        if user.is_staff or user.role in ["admin", "moderator"]:
        ...
        elif hasattr(user, "nouveau_role") and user.nouveau_role:
            # Vérifier le nouveau système RBAC si l'ancien rôle n'est pas défini
            ...
            # Enseignants et publiants vérifiés : soumission en attente de modération
                ...
                # Par défaut : en attente de modération pour les utilisateurs authentifiés
                validated_data["status"] = "pending"
        else:
            # Par défaut : en attente de modération
            validated_data["status"] = "pending"
...
class NewsDetailSerializer(NewsSerializer):
    ...
    admin_invalidated_by_name = serializers.CharField(
        source="admin_invalidated_by.get_full_name", read_only=True
    )

    class Meta(NewsSerializer.Meta):
        fields = NewsSerializer.Meta.fields + (
            "moderator_name",
            "admin_invalidated_by_name",
        )


class NewsModerationSerializer(serializers.ModelSerializer):
    """Sérialiseur pour la modération des news par les modérateurs"""
    ...
    def validate(self, data):
        """Valider que le titre et le contenu final sont fournis si approuvé"""
        if data.get("moderator_approved") and not data.get("final_title"):
            raise serializers.ValidationError(
                {"final_title": "Le titre final est requis lors de l'approbation"}
            )
        if data.get("moderator_approved") and not data.get("final_content"):
            raise serializers.ValidationError(
                {"final_content": "Le contenu final est requis lors de l'approbation"}
            )
        return data
    ...
    def update(self, instance, validated_data):
        ...
        instance.final_title = validated_data.get("final_title", instance.draft_title)
        instance.final_content = validated_data.get(
            "final_content", instance.draft_content
        )
        instance.moderator_approved = validated_data.get("moderator_approved", False)
        instance.moderation_comment = validated_data.get("moderation_comment", "")

        # Si approuvé, publier la news
        if instance.moderator_approved:
...
class NewsInvalidationSerializer(serializers.ModelSerializer):
    """Sérialiseur pour l'invalidation des news par les administrateurs"""

    class Meta:
        model = News
        fields = ("admin_invalidation_reason",)

    def validate_admin_invalidation_reason(self, value):
        """La motivation de l'invalidation est requise"""
    ...
    def update(self, instance, validated_data):
        ...
        # Invalider la news
        instance.admin_invalidated_by = self.context["request"].user
        instance.admin_invalidation_reason = validated_data.get(
            "admin_invalidation_reason", ""
        )
        instance.invalidated_at = timezone.now()
...
class UserProfilePreferencesSerializer(serializers.ModelSerializer):
    """Sérialiseur pour les préférences de notification des utilisateurs"""
    ...
    def validate_notification_frequency(self, value):
        """Valider la fréquence de notification"""
        valid_frequencies = ["immediate", "daily", "weekly", "disabled"]
        if value not in valid_frequencies:
            raise serializers.ValidationError(
                f'Fréquence invalide. Choisissez parmi: {", ".join(valid_frequencies)}'
            )
        return value
...
class DepartementSerializer(serializers.ModelSerializer):
    ...
    universite_nom = serializers.CharField(
        source="faculte.universite.nom", read_only=True
    )
    utilisateurs_count = serializers.SerializerMethodField()
    ...
    def validate(self, data):
        ...
        if data.get("faculte") and data.get("universite"):
            if data["faculte"].universite != data["universite"]:
                raise serializers.ValidationError(
                    "La faculté ne correspond pas à l'université."
                )

        if data.get("departement") and data.get("faculte"):
            if data["departement"].faculte != data["faculte"]:
                raise serializers.ValidationError(
                    "Le département ne correspond pas à la faculté."
                )
