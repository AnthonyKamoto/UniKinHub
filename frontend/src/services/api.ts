...
export const newsAPI = {
  ...
  createNews: async (data: NewsFormData | FormData): Promise<News> => {
    // Si c'est déjà un FormData, l'envoyer directement via le ViewSet
    if (data instanceof FormData) {
      const response = await api.post('/api/news-api/', data, {
        headers: {
    ...
    // Sinon, construire FormData avec le système de modération (draft_title/draft_content)
    const formData = new FormData()

    // Utiliser draft_title et draft_content pour le système de modération
    formData.append('draft_title', data.title)
    formData.append('draft_content', data.content)
    formData.append('category', data.category.toString())
    formData.append('importance', data.importance || 'medium')
    formData.append('programme_ou_formation', data.program || '')
    formData.append('desired_publish_start', new Date().toISOString())

    if (data.target_universities && data.target_universities.length > 0) {
      formData.append(
        'target_universities',
  ...
  updateNews: async (
    ...
    // Utiliser le ViewSet pour la mise à jour
  ...
  deleteNews: async (id: number): Promise<void> => {
    // Utiliser le ViewSet pour la suppression
  ...
  moderateNews: async (
    ...
    // Utiliser les endpoints de modération du ViewSet
    const endpoint = action === 'approve' ? 'approve' : 'reject'
    const payload = action === 'reject' && reason ? { reason } : {}

    const response = await api.post(`/api/news-api/${id}/${endpoint}/`, payload)
    return response.data
  },
...
export const commentAPI = {
  ...
  createComment: async (newsId: number, content: string): Promise<Comment> => {
  ...
  moderateComment: async (id: number, approved: boolean): Promise<Comment> => {
...
export const rbacAPI = {
  ...
  getUsers: async (params?: {
    ...
    const url = `/users/${searchParams.toString() ? `?${searchParams}` : ''}`
    const response = await api.get(url)
    return response.data
  },
