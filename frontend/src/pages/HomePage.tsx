...
const HomePage = () => {
  ...
  const handleDelete = async (newsId: number, e: React.MouseEvent) => {
    e.stopPropagation()
    if (
      window.confirm(
        '⚠️ Êtes-vous sûr de vouloir supprimer cette actualité ? Cette action est irréversible.'
      )
    ) {
      try {
        await newsAPI.deleteNews(newsId)
        // Retirer l'actualité de la liste locale
